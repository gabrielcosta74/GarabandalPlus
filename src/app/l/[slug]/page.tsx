...
export async function generateStaticParams(): Promise<Params[]> {
  const rows = await listAllSlugs('posts', 'pt');
  return rows.map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPostBySlug(slug, 'pt', PUBLISHED_ONLY);
  if (!post) return { title: 'Artigo não encontrado' };
  const isPreview = post.status !== 'published';

  const peers = await getTranslationPeers('post', post.id);
  const languages = buildHreflang('post', 'pt', slug, peers);
...
export default async function PostPage({ params }: { params: Promise<Params> }) {
  ...
  const peers = await getTranslationPeers('post', post.id);
  const moreRaw = await listPosts('pt', { limit: 7 });
  ...
  const ld = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${APP_URL}/l/${slug}` },
    inLanguage: 'pt',
  };
      ...
      <ArticleHero
        ...
        meta={
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            {peers.length > 1 ? <LocaleSwitcher current="pt" peers={peers} basePath="post" /> : null}
          </div>
        }
      />
        ...
        >
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              ...
              <Link href="/l" style={{ fontSize: '0.9rem', color: 'var(--primary, #1d4ed8)', textDecoration: 'underline', textUnderlineOffset: 3 }}>
                Ver todos
              </Link>
            </div>
            <div
