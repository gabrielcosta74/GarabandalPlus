...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const post = await getPostBySlug(slug, 'es', PUBLISHED_ONLY);
...
export default async function EsPostPage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <main>
      ...
      <ShareBar url={`${APP_URL}/es/l/${slug}`} title={post.title} locale="es" />
      <ShareCTA url={`${APP_URL}/es/l/${slug}`} title={post.title} locale="es" />
