...
export default function CheckinForm({ slug, title, dateLabel }: { slug: string; title: string; dateLabel: string }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error || 'Não foi possível enviar os dados.');
      setComplete(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (submissionError) {
      setError(submissionError instanceof Error ? submissionError.message : 'Não foi possível enviar os dados.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-10 sm:py-16">
        <div className="mx-auto max-w-xl rounded-3xl border border-emerald-200 bg-white p-7 text-center shadow-sm sm:p-10">
          ...
          <h1 className="mt-5 font-serif text-3xl font-bold text-slate-900">Dados enviados</h1>
      ...
      <div className="mx-auto max-w-3xl">
        <header className="rounded-3xl bg-slate-900 px-5 py-7 text-white shadow-lg sm:px-9 sm:py-9">
          <div className="flex items-center gap-2 text-sm font-semibold text-amber-300">
            ...
            Dados para check-in nos hotéis
          </div>
          <h1 className="mt-3 font-serif text-2xl font-bold leading-tight sm:text-4xl">{title}</h1>
        ...
        <form onSubmit={submit} className="mt-5 space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-9">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">Preencha os dados exatamente como aparecem no documento de identificação.</p>
          </div>
          ...
          <div className="border-t border-slate-200 pt-5">
            <h2 className="text-lg font-bold text-slate-900">Documento de identificação</h2>
          </div>
          ...
          <div className="border-t border-slate-200 pt-5">
            <h2 className="text-lg font-bold text-slate-900">Contacto e morada</h2>
          </div>
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
            ...
            />
            <span>
              Confirmo que os dados estão corretos e autorizo a sua utilização para organizar a peregrinação e efetuar o check-in nos hotéis. Consulte a{' '}
              <Link href="/privacidade" target="_blank" className="font-semibold underline underline-offset-2">Política de Privacidade</Link>.
            </span>
          </label>
          ...
          <p className="flex items-center justify-center gap-2 text-center text-xs text-slate-500">
            ...
            Os dados são enviados de forma segura e ficam acessíveis apenas à organização.
          </p>
        </form>
      </div>
    </main>
