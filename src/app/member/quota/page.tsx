...
export default function MemberQuotaPage() {
  ...
  const handleCheckout = async () => {
    if (!userId) {
      setError(isEn ? 'Invalid session. Please sign in again.' : 'Sessão inválida. Faz login novamente.');
      return;
    }
    if (showProfile) {
      setShowProfile(false);
    ...
    try {
      ...
      if (!url) throw new Error(isEn ? 'Unexpected response from the payment server.' : 'Resposta inesperada do servidor de pagamentos.');
  ...
  const nextQuotaPretty = nextQuota
    ? new Date(nextQuota).toLocaleDateString(isEn ? 'en-GB' : 'pt-PT', { day: '2-digit', month: 'long', year: 'numeric' })
    : (isEn ? 'No date' : 'Sem data');
  ...
  return (
    <DashboardShell
      title={isEn ? 'Annual Fee' : 'Quota Anual'}
      subtitle={isEn ? 'Check your subscription status or renew your annual fee.' : 'Verifica o estado da tua subscrição ou renova a quota.'}
    >
      <MemberProfileModal
        visible={showProfile}
        userId={userId}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-serif text-xl font-bold text-garabandal-dark">{isEn ? 'Current Status' : 'Estado Atual'}</h3>
            </div>

            <div className="grid grid-cols-2 gap-6 mb-8">
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">{isEn ? 'Status' : 'Estado'}</span>
                <div className={`text-lg font-bold flex items-center gap-2 ${statusPaid ? 'text-green-600' : 'text-amber-600'}`}>
                  ...
                  <span className="first-letter:uppercase">{
                    isEn 
                      ? (quotaStatusLabel === 'pago' ? 'paid' 
                         : quotaStatusLabel === 'expirado' ? 'expired' 
                         : quotaStatusLabel === 'pendente' ? 'pending' 
                         : quotaStatusLabel === 'fundador' ? 'founder' 
                         : 'undefined') 
                      : quotaStatusLabel
                  }</span>
                </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">{isEn ? 'Next Fee' : 'Próxima Quota'}</span>
          ...
          {canPay ? (
              ...
              <div>
                <p className={`font-bold text-sm ${isExpiredStatus || (!isMember && !!profileData?.numero_socio)
                  ...
                  {isExpiredStatus || (!isMember && !!profileData?.numero_socio)
                    ? (isEn ? 'Account Suspended' : 'Conta Suspensa')
                    : (isEn ? 'Renewal Required' : 'Renovação Necessária')}
                </p>
                <p className={`text-xs mt-1 ${isExpiredStatus || (!isMember && !!profileData?.numero_socio)
                  ...
                  {isExpiredStatus || (!isMember && !!profileData?.numero_socio)
                    ? (isEn ? 'Your account is suspended due to overdue fees. Regularize it now to reactivate access immediately.' : 'A sua conta está suspensa devido a quotas em atraso. Regularize agora para reativar o acesso imediato.')
                    : (isEn ? 'Your annual fee expires soon or is overdue. Renew now to keep your benefits.' : 'A tua quota expira em breve ou está em atraso. Renova agora para manteres os teus benefícios.')}
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-bold text-green-800 text-sm">{isEn ? 'Fee Up to Date' : 'Quota em Dia'}</p>
                <p className="text-green-600 text-xs mt-1">{isEn ? `Your subscription is active. Next renewal only on ${nextQuotaPretty}.` : `A tua subscrição está ativa. Próxima renovação apenas em ${nextQuotaPretty}.`}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {canPay && (
          <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-serif text-xl font-bold text-garabandal-dark">{isEn ? 'Payment' : 'Pagamento'}</h3>
            </div>

            <p className="text-sm text-gray-500 mb-6">{isEn ? 'Select your preferred payment method.' : 'Seleciona o método de pagamento preferido.'}</p>
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    <div className="font-bold text-gray-900 text-sm">
                      {isEn && option.id === 'reduniq_card' ? 'Credit Card' 
                        : isEn && option.id === 'reduniq_mbway' ? 'MB WAY' 
                        : isEn && option.id === 'reduniq_pix' ? 'PIX'
                        : isEn && option.id === 'reduniq_multibanco' ? 'Multibanco'
                        : option.label}
                    </div>
                    <div className="text-xs text-gray-500">
                      {isEn && option.id === 'reduniq_card' ? 'Visa / Mastercard' 
                        : isEn && option.id === 'reduniq_mbway' ? 'Instant payment' 
                        : isEn && option.id === 'reduniq_pix' ? 'Instant payment'
                        : isEn && option.id === 'reduniq_multibanco' ? 'Service payment'
                        : option.description}
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-between text-sm py-4 border-t border-gray-100 mb-4">
              <span className="text-gray-500">{isEn ? 'Annual Fee Amount' : 'Valor da Quota'}</span>
              <span className="font-bold text-xl text-garabandal-dark">{formatCurrency(QUOTA_AMOUNT, isEn ? 'en-GB' : 'pt-PT')}</span>
            </div>
            ...
            >
              {loading ? (isEn ? 'Processing...' : 'A processar...') : (isEn ? 'Pay Now' : 'Pagar Agora')}
            </button>
          </div>
        )}
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-serif text-xl font-bold text-garabandal-dark">{isEn ? 'Fee History' : 'Histórico de Quotas'}</h3>
          </div>
          ...
          {loadingData ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              <p className="text-gray-400 text-sm">{isEn ? "You don't have any recorded payments yet." : 'Ainda não tens pagamentos registados.'}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-gray-400 uppercase tracking-wider font-bold border-b border-gray-100">
                  <tr>
                    <th className="py-3 px-4">{isEn ? 'Date' : 'Data'}</th>
                    <th className="py-3 px-4">{isEn ? 'Amount' : 'Valor'}</th>
                    <th className="py-3 px-4">{isEn ? 'Status' : 'Estado'}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {payments.map((item) => (
                    <tr key={item.id || item.external_reference} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-gray-600">{formatCurrency(item.valor || 0, isEn ? 'en-GB' : 'pt-PT')}</td>
