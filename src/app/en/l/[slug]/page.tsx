...
export async function generateStaticParams(): Promise<Params[]> {
  const rows = await listAllSlugs('posts', 'en');
  return rows.map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPostBySlug(slug, 'en', PUBLISHED_ONLY);
  if (!post) return { title: 'Article not found' };
  const isPreview = post.status !== 'published';

  const peers = await getTranslationPeers('post', post.id);
  const languages = buildHreflang('post', 'en', slug, peers);
...
export default async function EnPostPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  const post = await getPostBySlug(slug, 'en', PUBLISHED_ONLY);
  if (!post) notFound();

  const peers = await getTranslationPeers('post', post.id);
  const moreRaw = await listPosts('en', { limit: 7 });
  ...
  const ld = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${APP_URL}/en/l/${slug}` },
    inLanguage: 'en',
  };

  const breadcrumb = breadcrumbJsonLd([
    { name: 'Home', path: '/en' },
    { name: 'Articles', path: '/en/l' },
    { name: post.title },
  ]);
      ...
      <ArticleHero
        ...
        meta={
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            {peers.length > 1 ? <LocaleSwitcher current="en" peers={peers} basePath="post" /> : null}
          </div>
        }
      />
      <ArticleBody html={post.content_html} />
      <ShareBar url={`${APP_URL}/en/l/${slug}`} title={post.title} locale="en" />
      <ShareCTA url={`${APP_URL}/en/l/${slug}`} title={post.title} locale="en" />
        ...
        >
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              ...
              <Link href="/en/l" style={{ fontSize: '0.9rem', color: 'var(--primary, #1d4ed8)', textDecoration: 'underline', textUnderlineOffset: 3 }}>
                See all
              </Link>
            </div>
            <div
