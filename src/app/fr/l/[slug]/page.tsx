...
export async function generateStaticParams(): Promise<Params[]> {
  const rows = await listAllSlugs('posts', 'fr');
  return rows.map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPostBySlug(slug, 'fr', PUBLISHED_ONLY);
  if (!post) return { title: 'Article introuvable' };
  const isPreview = post.status !== 'published';

  const peers = await getTranslationPeers('post', post.id);
  const languages = buildHreflang('post', 'fr', slug, peers);
...
export default async function FrPostPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  const post = await getPostBySlug(slug, 'fr', PUBLISHED_ONLY);
  if (!post) notFound();

  const peers = await getTranslationPeers('post', post.id);
  const moreRaw = await listPosts('fr', { limit: 7 });
  ...
  const ld = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${APP_URL}/fr/l/${slug}` },
    inLanguage: 'fr',
  };

  const breadcrumb = breadcrumbJsonLd([
    { name: 'Accueil', path: '/fr' },
    { name: 'Articles', path: '/fr/l' },
    { name: post.title },
  ]);
      ...
      <ArticleHero
        ...
        meta={
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            {peers.length > 1 ? <LocaleSwitcher current="fr" peers={peers} basePath="post" /> : null}
          </div>
        }
      />
      <ArticleBody html={post.content_html} />
      <ShareBar url={`${APP_URL}/fr/l/${slug}`} title={post.title} locale="fr" />
      <ShareCTA url={`${APP_URL}/fr/l/${slug}`} title={post.title} locale="fr" />
        ...
        >
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              ...
              <Link href="/fr/l" style={{ fontSize: '0.9rem', color: 'var(--primary, #1d4ed8)', textDecoration: 'underline', textUnderlineOffset: 3 }}>
                Voir tout
              </Link>
            </div>
            <div
