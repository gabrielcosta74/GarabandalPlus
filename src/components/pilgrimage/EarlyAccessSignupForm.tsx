...
export default function EarlyAccessSignupForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(copy.paths.confirm);
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : s.errorGeneric);
      setStatus('idle');
    }
  }
  ...
  if (status === 'submitting') {
    return (
      <div className="flex min-h-56 flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500" aria-live="polite">
        ...
        <p className="mt-7 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#d4bc7d]">{s.submittingTitle}</p>
        <p className="mt-3 text-sm text-white/45">{s.submittingSub}</p>
      </div>
    );
  }
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <p className="text-center text-[10px] leading-4 text-white/25">{s.disclaimer}</p>
    </form>
  );
}
