...
function buildSupportUrl(email: string) {
  const message = `Olá! Inscrevi-me no acesso antecipado do Caminho Mariano 2027 com o email ${email} e gostaria de receber a ligação do grupo privado.`;
...
export default function EarlyAccessConfirmation() {
  ...
  useEffect(() => {
    if (data === null) {
      router.replace('/acesso-antecipado');
    }
  }, [data, router]);

  async function shareEarlyAccess() {
    const url = `${window.location.origin}/acesso-antecipado`;
    const shareData = {
      title: 'Acesso VIP — Caminho Mariano 2027',
      text: 'Entra na lista privada para receber acesso às inscrições 48 horas antes.',
      url,
    };
  ...
  return (
    <div className="flex flex-1 flex-col justify-center py-10 text-center animate-in fade-in zoom-in-95 duration-500 sm:py-16">
      ...
      <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#bca56b]">
        {alreadyRegistered ? 'Email já registado' : 'Registo concluído'}
      </p>
      ...
      <p className="mx-auto mt-4 max-w-[22rem] text-sm leading-6 text-white/58">
        A 13 de outubro recebe o link de inscrição por email — dois dias antes da abertura pública a 15 de outubro.
      </p>
      ...
      <div className="mt-5 flex items-start justify-center gap-8 border-y border-white/10 py-5">
        <div className="text-center">
          ...
          <p className="mt-1.5 text-[10px] uppercase tracking-[0.18em] text-[#e0c37b]/70">O seu acesso</p>
        </div>
      ...
      <p className="mt-5 text-xs leading-5 text-white/40">
        Aceda já ao grupo para receber todos os detalhes da peregrinação.
      </p>
      ...
      >
        <span className="flex items-center gap-2.5">
          ...
          {whatsappUrl ? 'Aceder ao grupo privado' : 'Pedir ligação do grupo'}
        </span>
      ...
      >
        <span>{copied ? 'Ligação copiada' : 'Convidar alguém que conheço'}</span>
        {copied ? <Copy className="h-4 w-4 text-[#d4bc7d]" aria-hidden="true" /> : <Share2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <p className="mt-4 text-[10px] leading-4 text-white/32">Partilhe esta ligação com quem também quer receber acesso VIP.</p>
    </div>
  );
}
